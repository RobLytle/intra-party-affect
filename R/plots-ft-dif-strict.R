...
#Exactly the same as plots-ft-dif.R, but using a strict 50/70 cutoff. Originally, 30 was chosen for cold, but there simply are very few < 30 partisans
...
#
opinion_strict_strict_df <- read_rds("data/tidy-cdf.rds")%>%
...
opinion_boot_strict_strict_df <- data.frame(boot = 1:2000)%>%
	group_by(boot)%>%
	do(sample_n(opinion_strict_strict_df, nrow(opinion_strict_strict_df), replace = TRUE))%>% #creating 2000 new datasets of equal size to the original
	group_by(boot,
					 pid_3,
					 below_50_qual_strict)%>%
	summarize(
...
se_op_strict_strict_df <- opinion_boot_strict_strict_df%>% #putting this here so I don't have to run the resample every time. Once troubleshooted, I will put in one pipe
...
opinion_means_strict_strict_df <- read_rds("data/tidy-cdf.rds")%>%
	filter(year >= 1978 & pid_3 != "Independent")%>%
...
opinion_pooled_strict_df <- full_join(opinion_means_strict_df, se_op_strict_df)%>%
	mutate(which_question = reorder(which_question, prop_difference),
...
gg_strict_opinion_pooled <- ggplot(opinion_pooled_strict_df, aes(x = prop_difference, y = fct_relabel(which_question, str_wrap, width = 20))) +
	geom_linerange(aes(xmin = prop_difference - 1.645*prop_se, xmax = prop_difference + 1.645*prop_se, color = pid_3), position = dodge) +
#	geom_point(data=opinion_pooled_strict_df[opinion_pooled_strict_df$sig_dum == TRUE,],size=5, aes(position = pid_3), shape = 1, position = dodge) + #overlays a shape on sig 
...
###
behavior_strict_df <- read_rds("data/tidy-cdf.rds")%>%
	filter(year >= 1978 & pid_3 != "Independent")%>%
select(weight, #just selecting the relevant vars because the resampling takes a long time.
			 pid_3,
			 below_50_qual_strict,
...
behavior_boot_strict_df <- data.frame(boot = 1:2000)%>%
	group_by(boot)%>%
	do(sample_n(behavior_strict_df, nrow(behavior_strict_df), replace = TRUE))%>%
	group_by(boot,
					 pid_3,
					 below_50_qual_strict)%>%
	summarize(
...
ci_strict_df <- behavior_boot_strict_df%>% #putting this here so I don't have to run the resample every time
	filter(!is.na(below_50_qual_strict))%>%
	pivot_wider(names_from = below_50_qual_strict,
...
###
behavior_means_strict_df <- read_rds("data/tidy-cdf.rds")%>%
	filter(year >= 1978 & pid_3 != "Independent")%>%
	#	glimpse()
...
behavior_pooled_strict_df <- full_join(behavior_means_strict_df, ci_strict_df)%>%
	mutate(which_question = reorder(which_question, prop_difference),
				 sig_dum = case_when(prop_difference < 0 & prop_difference + 1.645*prop_se < 0 ~ TRUE,
				 										 prop_difference > 0 & prop_difference - 1.645*prop_se > 0 ~ TRUE,
				 										 TRUE ~ FALSE))%>%
...
gg_strict_behavior_pooled <- ggplot(behavior_pooled_strict_df, aes(x = prop_difference, y = fct_relabel(which_question, str_wrap, width = 20))) +
	geom_linerange(aes(xmin = prop_difference - 1.645*prop_se, xmax = prop_difference + 1.645*prop_se, color = pid_3), position = dodge) +
#	geom_point(data=behavior_pooled_strict_df[behavior_pooled_strict_df$sig_dum == TRUE,],size=5, aes(position = pid_3), shape = 1, position = dodge) + #overlays a shape on sig 
	geom_point(aes(color = pid_3, shape = pid_3), size = 3, position = dodge) +
		scale_color_manual(values = c("Democrat" = "dodgerblue3",
																"Republican" = "firebrick3")) +
	geom_vline(xintercept = 0.00) +
...
gg_pooled_combined
ggsave("fig/gg-pooled-combined-strict.png", gg_pooled_combined, width = 6, height = 12, units = "in")
