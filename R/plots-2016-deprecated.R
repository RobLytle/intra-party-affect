...
base_therm_hist_2016 <- ggplot(df_primary, aes(x = net_party_affect, y = ..density.., fill = primary_vote_choice, alpha = .1)) + 
  guides(alpha = FALSE) +
  geom_density() +
  theme_bw()+
...
df_2008_primary <- df_primary%>%
  filter(year == 2008)%>%
  glimpse


df_2016_primary <- df_primary%>%
  filter(year == 2016)%>%
  glimpse()

mean(df_primary$pre_therm_dem, na.rm = TRUE)

scat_08 <- ggplot(df_2008_primary, aes(pre_therm_dem, pre_therm_rep, alpha = .4))+
  facet_wrap(vars(primary_vote_choice))+
  guides(alpha = FALSE) +
  geom_vline(aes(xintercept = mean(df_2008_primary$pre_therm_dem, na.rm = TRUE))) +
  geom_point()+
  theme_bw()+
  theme(
    plot.background = element_blank(),
...
scat_16 <- ggplot(df_2016_primary, aes(pre_therm_dem, pre_therm_rep, alpha = .4))+
  facet_wrap(vars(primary_vote_choice))+
  guides(alpha = FALSE) +
  geom_vline(aes(xintercept = mean(df_2016_primary$pre_therm_dem, na.rm = TRUE))) +
  geom_point()+
  theme_bw()+
  theme(
  plot.background = element_blank(),
...
primary_scats <- grid.arrange(scat_08,
                              scat_16,
                              nrow = 2)
primary_scats

ggsave("fig/primary-scatter.png", plot = primary_scats, width = 8, height = 8, units = "in")
#####
...
#ideo_hist <- ggplot(df_primary, aes(x = pre_self_ideo_7, alpha = .1)) + 
#  geom_bar() +
#  theme_bw()+
...
#sum(is.na(df_2016$parties_ideo_dif))

#ggsave("fig/party-ideo-dif.png", width = 10, height = 6, units = "in")


#fit_bs <- lm(subset = primary_vote_choice == "Bernie Sanders", parties_therm_dif ~ parties_ideo_dif, data = df)


#ggplot(df, aes(parties_ideo_dif, parties_therm_dif))+
#  geom_point(position = "jitter")+
#  geom_smooth(method = "lm")



#ggplot(df_2016, aes(x = parties_therm_dif)) +
