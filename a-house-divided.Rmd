...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, fig.width = 6, fig.height = 4, fig.pos = "H", fig.showtext = TRUE, fig.cap = TRUE)
library(lubridate)
#library(MASS)
library(brglm)
...
Particularly in the Democratic Party context it is difficult to square the ideological polarization proposed by @groenendyk2020 with the qualitative features of the visible power struggle within the party. I propose an alternative (or perhaps supplementary) explanation: that intra-party contests, particularly primary elections, complicate the in-party/in-group dynamic. Ultimately, weakening the in-party affinity and increasing the political disdain felt by those who find themselves on the losing side of these contests.
...
```{r cdf-means, fig.cap="Time-series shifts in partisan affect.", fig.showtext=TRUE}
...
cm_3 <- c("primary_vote_simpleLoser" = "Primary Loss",
					"primary_vote_simpleDidn't Vote" = "Didn't Vote",
...
primary_tab<-df_primaries%>%
  ...
  group_by(group_name,
           ...
           therm_name)%>%
  nest_by() %>% 
  mutate(fit = list(lm(therm_val ~ primary_vote_simple + ideo_self_in_dif_std + age_group + year, data = data,  weights = weight)))%>%
...
tab_header(title = md("**Table 1:** Estimated Individual Partisan Affect")) %>% 
  tab_footnote(md("Binary variables for election year and respondent age are included to \ control for shifts in affect exogenous to primary outcome. Those who view themselves as more extreme than their party are also warmer towards the party than those who view themselves as more moderate than the party."),
...
gg_strict_pooled_combined_si <- grid.arrange(arrangeGrob(pg,
																							 top = text_grob("Prop. Difference Between Cold and Warm Partisans", vjust = 1, face = "bold"),
																							 left = text_grob("Question Asked", rot = 90, vjust = 1, face = "bold"),
																							 bottom = text_grob("Cold - Warm", face = "bold")))
```

```{r fig-display-si, include=FALSE, fig.height=8.5, fig.width=4, fig.align='center', fig.caption = "Differences in attitude and behavior between cold and warm partisans."}
as_ggplot(gg_strict_pooled_combined_si)
...
```{r primaries-table-si, include=FALSE}
gm_4 <- tibble::tribble(
  ~raw,        ~clean,          ~fmt,
  "nobs",      "N",             0,
  "r.squared", "R Sq.", 2,
...
# not including ideology var because this  is probably just a proxy for partisanship
cm_4 <- c("primary_vote_simpleLoser" = "Primary Loss",
          "year2012" = "2012",
          "year2016" = "2016", 
          "year2020" = "2020",
          "(Intercept)" = "Constant") 
...
df_primaries%>%
  ...
  tab_spanner(columns = 2:3, label = "In-Party") %>% 
  tab_spanner(columns = 4:5, label = "Out-Party")%>%
tab_header(title = md("**SI Table 1:** Changes in Average Partisan Feeling Thermometer <br> Conditional on Primary Loss")) %>% 
  tab_footnote(md("Survey-year dummy variables included to \ control for shifts in affect exogenous to primary outcome."),
               locations = cells_title())
```


!["Relationship between strength of partisanship and in-party feeling thermometers"](fig/gg-str-therm.png) 

***Fig SI-2:** There is a positive relationship between partisans' strength of identification and their in-party feeling thermometer ratings.*


```{r primaries-plots-si, include=FALSE, echo=FALSE,warning=FALSE, fig.cap="Distribution of partisan affect between primary candidate supporters 2008--2020"}
gg_primary_in <- ggplot(df_primaries, aes(x = therm_inparty, color = primary_vote_simple, linetype = primary_vote_simple)) +
	geom_density(size = .9,
							 alpha = 1) +
	facet_grid(rows = c(vars(year)), 
...
as_ggplot(grid.arrange(arrangeGrob(p2, p1,
																				 ncol = 2,
																				 top = text_grob("Distribution of Partisan Warmth\n by Primary Vote", vjust = 1, face = "bold"),
																				 left = text_grob("Density", rot = 90, vjust = 1, face = "bold"),
																			   bottom = text_grob("Feeling Thermometer", face = "bold"))))
