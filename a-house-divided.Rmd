...
fig_3_df <- df_primaries %>% 
	select(primary_vote_simple,
				 year,
				 pid_3,
				 therm_inparty) %>%
	mutate(cold = if_else(therm_inparty < 50, 1, 0)) %>% 
	group_by(primary_vote_simple,
					 pid_3,
					 year) %>% 
	summarize(pct_50_below = 100*mean(cold, na.rm = TRUE),
						n = n()) %>% 
	mutate(year = as.numeric(as.character(year))) %>% 
	ungroup() %>%
	filter(primary_vote_simple != "Voted in Other Primary" & !is.na(primary_vote_simple)) %>%
	mutate(prim_vote = as.factor(primary_vote_simple),
				 prim_vote = recode(prim_vote,
														"Didn't Vote" = "Non-voter")) %>% 
	glimpse() 


ggplot(fig_3_df, aes(x = year, y = pct_50_below, color = pid_3)) +
	facet_wrap(vars(primary_vote_simple)) +
#	 geom_linerange(aes(ymin = (pct_50_below - 1.96*se), ymax = (pct_50_below + 1.96*se)), position = dodge) +
  geom_line(aes(linetype = pid_3), size = 1, position = dodge) +
...
scale_color_manual(values = c("Republican" = "#481567FF",
 ...
 scale_shape_manual(values = c("Republican" = 15,
  															"Democrat" = 16)) +
	scale_x_continuous(breaks = c(seq(2008, 2020, by = 4))) +
	# scale_y_continuous(breaks = c(seq(0,25, by = 5)),
										 # limits = c(0,25)) +
	theme(legend.position = 'bottom',
