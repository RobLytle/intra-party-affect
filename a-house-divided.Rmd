...
thanks: "In no particular order, I express my gratitude for the feedback, guidance, and support provided by Brad Gomez, Hans Hassell, Doug Ahler, Jens Grosser, Kai Ou, Bob Jackson, Quintin Beazer, Gaurav Sood, Sarah Warren, Damian Boldt, Taylor Chewning, Olivia VanHousen, Braeden McNulty, William Thyer, and Kate Peirce. All advice is invaluable and all mistakes are my own."
header-includes:
...
prop_cold_df <- mct_prop_ns %>% 
	select(year,
				 group = pid_3,
				 prop_50_below,
				 se_50_below) %>% 
	glimpse()
gg_mean_ft_ns <- party_fts_ns_joined %>%
	filter(str_detect(group, "In")) %>% 
	mutate(group = str_remove(group, " - In Party")) %>% 
	left_join(prop_cold_df) %>% 
	mutate(mean = mean/100) %>%  #just recoding mean between 0/1 so that prop can be on
glimpse() %>% 
ggplot(aes(x = year, linetype = group, color = group, shape = group), position = dodge) +
#  geom_smooth(aes(linetype = group, color = group), span = .3, se=F) +
  geom_line(aes(y = mean)) +
	geom_line(aes(y = prop_50_below )) +
#  geom_linerange(aes(ymin = mean-sd, ymax = mean+sd, color = group), position = dodge) +
  #geom_point(aes(shape = group, color = group) position = dodge) +
	geom_point(aes(y = mean), size = 3) +
	geom_point(aes(y = prop_50_below , size = 3)) +
  ...
  scale_y_continuous(name = "Mean",
  									 ...
  									 sec.axis = sec_axis(breaks = seq(0, 1, by = .1), limits = c)) +
  labs(#y = "Mean",
       x = "Year",
...
```{r fig-display, fig.height=8.5, include=TRUE, fig.width=4, fig.align='center', fig.caption = "Differences in attitude and behavior between cold and warm partisans."}
